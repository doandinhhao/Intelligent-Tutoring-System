...
export const WaiterPage = () => {
  ...
  const refreshTablesAndHandoff = async () => {
    ...
    if (!selectedTableId && tableData.length > 0) {
    ...
    <main className="grid-waiter">
            ...
            return (
              <div key={item.order_item_id} className="order-row">
                <div>
                  ...
                  {canRequestChange ? (
                    <div className="change-request-box">
                      ...
                      >
                        {busy === `change-${item.order_item_id}` ? "Sending..." : "Request change"}
                      </button>
                    </div>
                  ) : null}
                </div>
                ...
                <div className="order-row-right">
                  <span className={`status-pill ${item.kitchen_status_key}`}>{item.kitchen_status_key}</span>
      ...
      <section className="panel">
        <div className="panel-header">
          ...
          <span className="badge">
            Ready: {readyItemsForSelectedTable.length} | Served: {servedItemsForSelectedTable.length}
          </span>
        </div>
